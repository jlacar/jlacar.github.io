...
1. It helped me adopt an [egoless programming](https://en.wikipedia.org/wiki/Egoless_programming) mindset. When I see code its own entity, I can separate my "self" (ego) from it. With less ego attached to the code, I can take criticism of the code more objectively. Once an idea is expressed by the code, it has to stand on its own merits. This makes me less likely to get defensive and it makes collaborative development like pair and mob programming much better. It also opens up many doors to refactoring because I can be more objectively ruthless with the code.
2. I often tell people to read the code out loud, to give it a voice. There's something about actually hearing an idea expressed out loud versus having our "inner voice" say it in our heads. Our inner voice subconsciously provides context to help us understand things. An external voice, especially one that's not our own, doesn't do that. It has to provide more context and meaning to the idea for it to be understood properly. People like to quote Llewellyn Falco's assertion that "for an idea to go from your head to the computer, it MUST go through someone else's hands." I have found the converse to be also true: "For an idea to go from the computer to your head, it MUST go through someone else's mouth."
3. Thinking of the code as someone who has something to say helps me build sensitivity and empathy for how others might interpret the code. When I imagine myself sitting across the table from the code as it explains itself to me, I can easily imagine someone else sitting in my place, listening to the code. Will they understand what the code is saying if they didn't know what I know about it? If not, how can the code tell its story so they would?

There are a few more things I wanted to cover but this is getting pretty long and it's Christmas morning, so I'll end here for now.
